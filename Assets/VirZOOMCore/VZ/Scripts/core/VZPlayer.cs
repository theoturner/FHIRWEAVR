...
#endif
{
   ...
   protected virtual void UpdateSetup()
   {
      ...
      else if (!Controller.IsBikeConnected())
      {
         ...
         release.text += "HOLD L+R TO BEGIN";

#if UNITY_EDITOR
      if (!Controller.IsBikeConnected())
      {
         if (Controller.ControllerName() == "Keyboard")
            release.text += "\n\n(or hold enter and backspace to play without bike)";
         else if (Controller.ControllerName() == "GearVR")
            release.text += "\n\n(or hold touchpad and trigger to play without bike)";
         else if (Controller.ControllerName() == "Gvr")
            release.text += "\n\n(or hold touchpad and app button to play without bike)";
         else
            release.text += "\n\n(or hold L1 and R1 to play without bike)";
      }
#endif

      // Hold both buttons to calibrate
      if (Controller.LeftButton.Held(0.1f) && Controller.RightButton.Held(0.1f) && Controller.IsHeadTracked())
      {
   ...
   const string kReleaseText = "By continuing use of the VirZOOM bike you agree to all terms and conditions of the License Agreement at virzoom.com/eula.htm";
